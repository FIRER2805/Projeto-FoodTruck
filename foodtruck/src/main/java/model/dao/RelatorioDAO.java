...
public class RelatorioDAO {
	public ArrayList<VendasCanceladasDTO> gerarRelatorioVendasCanceladasDAO() {
		...
		String query = "SELECT u.nome as NOME, " + 
				...
				"(SELECT SUM(p.preco * itv.quantidade) FROM itemvenda itv, produto p WHERE itv.idvenda = v.idvenda and itv.idproduto = p.idproduto GROUP BY idvenda) AS SUBTOTAL, " + 
				"v.taxaentrega as TAXA_ENTREGA, " + 
				"(v.TAXAENTREGA + (SELECT SUM(p.preco * itv.quantidade) FROM itemvenda itv, produto p WHERE itv.idvenda = v.idvenda and itv.idproduto = p.idproduto GROUP BY idvenda)) as TOTAL " + 
